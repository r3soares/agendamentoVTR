...
class MainStore {
  ...
  void getAgendasOcupadas() async {
    ...
    try {
      List<Agenda> agendas = (await _repoAgenda.findByPeriodo(Constants.formatoData.format(DateTime.now()),
              Constants.formatoData.format((DateTime.now().add(Duration(days: 365))))))
          .model;
      Map<String, Agenda> mapAgendas = {for (var item in agendas) item.data: item};
